const { expect, describe, beforeAll, afterAll, beforeEach, test } = require('@jest/globals')
...
describe('logs', () => {
  ...
  test('GET /api/projects/logs/:_id?itemId=X Annotate creates logs', async () => {
    ...
    return supertest(app)
      ...
      .then((res) => {
        ...
        expect(res.body.data.find((log) => log.annotations.length === 1 && log.type === 'annotation-add')).toBeTruthy()
        expect(
          res.body.data.find((log) => log.annotations.length === 1 && log.type === 'annotation-remove')
        ).toBeTruthy()
        expect(res.body.data.find((log) => log.annotations.length === 1 && log.type === 'annotation-add')).toBeTruthy()
      })
  })
  ...
  test('GET /api/projects/logs/:_id?itemId=X Annotate with relations creates logs', async () => {
    ...
    const relationRemovedLog = res.body.data.filter((obj) => {
      return obj.type === 'relation-remove'
    })
    const relationAddLog = res.body.data.filter((obj) => {
      return obj.type === 'relation-add'
    })

    const annotationRemoveLog = res.body.data.filter((obj) => {
      return obj.type === 'annotation-remove'
    })
    const annotationAddLog = res.body.data.filter((obj) => {
