...
const getFileFromZip = async (filename, res) => {
  const zip = await unzip.Open.buffer(res.body)
  const annotations = zip.files.find((f) => f.path === filename)
  return (await annotations.buffer()).toString()
}

const getConfigFromZip = async (res) => {
  const str = await getFileFromZip('config.json', res)
  return JSON.parse(str)
}

const getLinesFromZip = async (filename, res) => {
...
describe('projects', () => {
  describe('POST /api/projects/import', () => {
    ...
    describe('add new annotations', () => {
      ...
      test('logs should be created', async () => {
        ...
        expect(annotationLog.annotations).toHaveLength(8)
      })
    })
  ...
  describe('PUT /api/projects/:id', () => {
    ...
    test('Update one task, and create 3 new', async () => {
      ...
      const name = res.body.tasks.find((obj) => obj.value === 'bbox_name')
      const foo = res.body.tasks.find((obj) => obj.value === 'foo')
      const bar = res.body.tasks.find((obj) => obj.value === 'bar')
  ...
  test('GET /api/projects/:_id/exports Export should not contain superfluous properties', async () => {
    ...
    expect(Array.isArray(json.tasks)).toBeTrue()
  ...
  test('GET /api/projects/:_id/exports?annotationsAndComments=true Export should contain NER-relations annotations', async () => {
    ...
    const [annotationsExport, relationsExport] = Object.values(json.annotation.ner)
